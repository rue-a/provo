...
class ProvGraph(Graph):
    ...
    def addProcess(self,  ID, processType):
        ...
        # if proc with this name(ID) is already in graph, generate new Instance by appending
        # running numbers. therefore addProc gives the ID of the instance back
        counter = 1
        while ID  in self.uniqueProcs:
            ID = URIRef(str(ID) + str(counter))
            counter += 1
        else:            
            self.uniqueProcs.append(ID)
